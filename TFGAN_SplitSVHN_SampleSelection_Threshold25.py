...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.totalTestingSetY = totalTestY
    ...
    def Create_Teacher(self, index, selectedComponentIndex):
        ...
        reco = Generator_SVHN(decoderName, code, reuse=True)
    ...
    def GenerateByCount(self,n):
        ...
        for i in range(count):
            batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
            gen = self.sess.run(self.GeneratorArr[0],feed_dict={self.z:batch_z})
            for j in range(self.batch_size):
                arr.append(gen[j])
        arr = np.array(arr)
        return arr
        ...
        with tf.Session(config=config) as sess:
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if t1%100 == 0:
                    #reco
                    '''
                    A1 = self.cifar_test_x[0:self.batch_size]
                    reco1 = self.sess.run(self.stu_inferenceReco,feed_dict={self.inputs:A1})
                    reco1 = reco1 * 255.0
                    reco1 = np.reshape(reco1,(-1,32,32,3))
                    str2 = "TT_VAEGAN_Reco_" + str(t1) + ".png"
                    cv2.imwrite(os.path.join("results/", str2),
                                merge2(reco1[:64], [8, 8]))
            ...
            #save the model
            #self.saver.save(self.sess, "save/TFGAN_SplitCIFAR10.ckpt")

            print("Memory size")
            print(np.shape(self.TotalSTMX)[0])
            print("acc")

            reco6, recoLoss6,realBatch = self.Calculate_Student_RecoLoss(self.svhn_test_x)
            print(recoLoss6)

            print("FID evaluation")

            fid6 = self.Calculate_FID_Results(self.svhn_test_x)
            print(fid6)

            print("IS score")
            is1 = self.Calculate_IS_Results(self.svhn_test_x)
            print(is1)
            ...
            for i in range(self.componentCount):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "TT__Final_SVHN_" + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
