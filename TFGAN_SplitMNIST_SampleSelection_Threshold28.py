...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        labelArr1_test = labelArr1_test
        labelArr2_test = labelArr2_test
        labelArr3_test = labelArr3_test
        labelArr4_test = labelArr4_test
        labelArr5_test = labelArr5_test

        data_textX = np.concatenate((arr1_test, arr2_test, arr3_test, arr4_test, arr5_test), axis=0)
        data_textY = np.concatenate(
            (labelArr1_test, labelArr2_test, labelArr3_test, labelArr4_test, labelArr5_test), axis=0)
    ...
    def Create_Teacher(self, index, selectedComponentIndex):
        ...
        gen_logits = Generator_mnist(decoderName, self.z, reuse=False)
        gen_logits = tf.reshape(gen_logits, (-1, 28, 28, 1))
        self.GeneratorArr.append(gen_logits)

        #build inference models
        #code = Encoder_SVHN_Static(self.inputs, Stu_InferenceName, batch_size=64, reuse=False)
        z_mean, z_log_sigma_sq,_,_ = Encoder_mnist(self.inputs, Stu_InferenceName, batch_size=64, reuse=False)

        self.vae_mean = tf.reduce_mean(z_mean,axis=0)
        self.vae_sigma = tf.reduce_mean(z_log_sigma_sq,axis=0)
        ...
        reco = Generator_mnist(decoderName, code, reuse=True)
        reconstruction_loss1 = tf.reduce_mean(tf.reduce_sum(tf.square(reco - self.inputs), [1, 2, 3]))

        self.evaluation_score = tf.reduce_sum(tf.square(reco - self.inputs), [1, 2, 3])
        ...
        vaeLoss = reconstruction_loss1 + KLThreshold*KL_divergence1

        _,D_real_logits,_ = Discriminator_Mnist(self.inputs, discStr, reuse=False)

        # output of D for fake images
        _,D_fake_logits1,_ = Discriminator_Mnist(gen_logits, discStr, reuse=True)
        ...
        _,d_hat,_ = Discriminator_Mnist(x_hat, discStr, reuse=True)
        scale = 10.0
        ddx = tf.gradients(d_hat, x_hat)[0]
        ddx = tf.sqrt(tf.reduce_sum(tf.square(ddx), axis=1))
        ddx = tf.reduce_mean(tf.square(ddx - 1.0) * scale)
        ...
        stuParameters = [var for var in T_vars if var.name.startswith(Stu_InferenceName)]

        VAE_parameters = var3 + stuParameters
        ...
        with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
            ...
            self.stuInference_optim1 = tf.train.AdamOptimizer(learning_rate=0.0001) \
                .minimize(vaeLoss, var_list=stuParameters)
    ...
    def build_model(self):
        ...
        self.inputs = tf.placeholder(tf.float32, [self.batch_size, 28, 28, 1])
    ...
    def FID_Evaluation(self, recoArr, test):
        ...
        recoArr = np.tile(recoArr,(1,1,1,3))
        test = np.tile(test,(1,1,1,3))
    ...
    def Calculate_Loss(self,batch1):
        sum1 = self.sess.run(self.evaluation_score, feed_dict={self.inputs: batch1, self.inputs: batch1})
        return sum1

    def GiveGenerativeSamples(self,n):
        arr = []
    ...
    def Calculate_MeanAndSigma(self,arrX):
        mean1,sigma = self.sess.run([self.vae_mean,self.vae_sigma],feed_dict={self.inputs:arrX})
        return mean1,sigma

    def Calculate_JS_Distance(self,mean1,sigma1,mean2,sigma2):
        #mean1 = mean1.astype(np.float64)
        #mean2 = mean2.astype(np.float64)
        with tf.Session() as sess:
            d1 = tfd.Normal(loc=mean1, scale=sigma1)
            d2 = tfd.Normal(loc=mean2, scale=sigma2)
            a1 = tfd.kl_divergence(d1,d2)
            a2 = tfd.kl_divergence(d1,d2)
            r = 0.5*a1 + 0.5 * a2
            r = sess.run(r)
            r = np.mean(r)
            return r
    ...
    def SampleSelection(self):
        ...
        blockMemoryX = self.FixedMemory[0:self.block_num_samples]
        blockMemoryY = self.FixedMemoryLabel[0:self.block_num_samples]

        return blockMemoryX,blockMemoryY
        ...
        with tf.Session(config=config) as sess:
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                    if np.shape(self.STM_ArrX)[0] == 0:
                        blockX,blockY = self.SampleSelection()

                        mean1,sigma1 = self.Calculate_MeanAndSigma(blockX)

                        self.blockMeanArr.append(mean1)
                        self.blockSigmaArr.append(sigma1)

                        self.STM_ArrX.append(blockX)
                        self.STM_ArrY.append(blockY)

                        self.TotalSTMX = blockX
                        self.TotalSTMY = blockY
                        ...
                        #Check the expansion
                        print("Check_Memory")
                        newMean,newSigna = self.Calculate_MeanAndSigma(newX)
                        arr = []
                        for j in range(np.shape(self.STM_ArrX)[0]):
                            mean3 = self.blockMeanArr[j]
                            sigma3 = self.blockSigmaArr[j]

                            dis = self.Calculate_JS_Distance(mean3,sigma3,newMean,newSigna)
                            arr.append(dis)
                        maxvalue = np.max(arr)
                        ...
                        if maxvalue > self.threshold:
                            ...
                            self.STM_ArrX.append(blockX)
                            self.STM_ArrY.append(blockY)
                            self.TotalSTMX = np.concatenate((self.TotalSTMX,blockX),axis=0)
                            self.TotalSTMY = np.concatenate((self.TotalSTMY,blockY),axis=0)
                ...
                if t1 < totalCount2:
                    ...
                    ucount = 0
                    umaxCount = self.num_training
                    trainSetX = self.FixedMemory
                    trainSetY = self.FixedMemoryLabel

                    if np.shape(self.TotalSTMX)[0] > 0:
                        trainSetX = np.concatenate((trainSetX,self.TotalSTMX),axis=0)
                        trainSetY = np.concatenate((trainSetY,self.TotalSTMY),axis=0)

                    n_examples_2 = np.shape(trainSetX)[0]
                    index2 = [i for i in range(n_examples_2)]
                    np.random.shuffle(index2)
                    trainSetX = trainSetX[index2]
                    trainSetY = trainSetY[index2]

                    for x_batch,y_batch in train_datagen.flow(trainSetX,trainSetY, batch_size=64, shuffle=True):
                        ucount = ucount +1
                        ...
                        if np.shape(x_batch)[0] == 64:
                            ...
                            _, g_loss,_ = self.sess.run(
                                [self.g_optim1, self.g_loss1,self.stuInference_optim1],
                    ...
                    for x_batch, y_batch in train_datagen.flow(trainSetX, trainSetY, batch_size=64,
                                                               shuffle=True):
                        ucount = ucount + 1
                ...
                if t1%100 == 0:
                    ...
                    newGan = peosoData * 255.0
                    newGan = np.reshape(newGan, (-1, 28, 28, 1))
                    str2 = "MyTT_VAEGAN_MNIST" + str(t1) + ".png"
                    cv2.imwrite(os.path.join("results/", str2),
                                merge2(newGan[:64], [8, 8]))
                    '''
            ...
            #
            #save the model
            self.saver.save(self.sess, "save/TFGAN_SplitMNIST.ckpt")

            print("Memory size")
            print(np.shape(self.STM_ArrX)[0]*self.block_num_samples)
            #print("acc")
            print(np.shape(self.STM_ArrX)[0])

            reco6, recoLoss6,realBatch = self.Calculate_Student_RecoLoss(self.mnist_Test)
            #print(recoLoss6)

            print("FID evaluation")

            print(np.shape(self.STM_MemoryX)[0])

            fid6 = self.Calculate_FID_Results(self.mnist_Test)
            print(fid6)

            print("IS score")
            is1 = self.Calculate_IS_Results(self.mnist_Test)
            print(is1)

            print("Generation")

            generated = self.GiveGenerativeSamples(5000)
            totalTest = self.mnist_Test[0:np.shape(generated)[0]]

            fid6 = self.FID_Evaluation(totalTest, generated)
            print(fid6)

            print("IS score")
            generated = np.tile(generated, (1, 1, 1, 3))
            is1 = self.Calculate_IS(generated)
            print(is1)

            realBatch1 = realBatch * 255.0
            recoBatch = reco6 * 255.0

            str1 = "TT_MNIST_real" + str(self.threshold) + ".png"
            cv2.imwrite(os.path.join("results/", str1),
                        merge2(realBatch1[:64], [8, 8]))

            str1 = "TT_MNIST_reco" + str(self.threshold) + ".png"
            cv2.imwrite(os.path.join("results/", str1),
                        merge2(recoBatch[:64], [8, 8]))
            ...
            for i in range(self.componentCount):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 28, 28, 1))
                str2 = "TT_MNIST_SixTask_" + str(i) + "_" + str(self.threshold)+ ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
